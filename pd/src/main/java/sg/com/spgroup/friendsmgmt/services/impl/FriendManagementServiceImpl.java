...
public class FriendManagementServiceImpl implements FriendManagementService {
    ...
    @Override
    public boolean createUserProfile(final String emailId, final String fullname) throws ApplicationException {
	...
	try {
	    ...
	    throw new GeneralApplicationException(String.format("There was an error creating profile for %s", emailId));
	}
	...
	FriendConnection friendConnection = connectionRepo.findByConnectionUUID1AndConnectionUUID2(fromProfile.getId(),
		toProfile.getId());

	if (friendConnection != null) {
	    throw new FriendConnectionAlreadyExists(fromEmailId, toEmailId);
	} else {
	    friendConnection = connectionRepo.findByConnectionUUID1AndConnectionUUID2(toProfile.getId(),
		    fromProfile.getId());
	...
	FriendBlock block = blockRepo.findByProfileUUIDAndBlockedUUID(fromProfile.getId(), toProfile.getId());

	if (block != null) {
	    throw new UserBlockedException(fromEmailId, toEmailId);
	}

	FriendConnection newFriendConnection = new FriendConnection(fromProfile.getId(), toProfile.getId());

	try {
	    connectionRepo.save(newFriendConnection);
	} catch (Exception e) {
	    LOGGER.error(e.getMessage());
	    throw new GeneralApplicationException(String
		    .format("There was an error making friend connection between %s and %s", fromEmailId, toEmailId));
	}
    ...
    @Override
    public FriendsMgmtResponse getFriendConnections(final String emailId) throws ApplicationException {
	FriendsMgmtResponse response = new FriendsMgmtResponse();
    ...
    @Override
    public boolean createFriendSubscription(final String profileEmailId, final String subscriberEmailId)
	    throws ApplicationException {
	...
	FriendSubscriber newSubscription = new FriendSubscriber(profile.getId(), subscriber.getId());

	try {
	    subscriberRepo.save(newSubscription);
	} catch (Exception e) {
	    LOGGER.error(e.getMessage());
	    throw new GeneralApplicationException(String.format("There was an error making subscription from %s to %s",
    ...
    @Override
    public FriendsMgmtResponse getCommonFriendConnections(final String fromEmailId, final String withEmailId)
	    throws ApplicationException {
	FriendsMgmtResponse response = new FriendsMgmtResponse();
	...
	List<String> friendsWith = connectionRepo.findFriendListByConnectionUUID1(profile.getId());
	...
	try {
	    ...
	    LOGGER.error(e.getMessage());
	    throw new GeneralApplicationException(String
		    .format("There was an error blocking subscription from %s to %s", profileEmailId, blockedEmailId));
	}
	return true;
    }

    @Override
    public FriendsMgmtResponse postUpdate(final String profileEmailId, final String text) throws ApplicationException {
	// TODO Auto-generated method stub
